		...
		case "up": 
			if(currentCell.getRow()-1>=0) {
				if(currentCell.isLinked(currentCell.getNorth())) {
					currentCell=cellsArr[currentCell.getRow()-1][currentCell.getColumn()];
					return true;
				}
			
			}
			return false;
		case "down":if(currentCell.getRow()+1<rows) {
		...
		case "left":
			if(currentCell.getColumn()-1>=0) {
				if(currentCell.getWest().isLinked(currentCell)) {
					currentCell=cellsArr[currentCell.getRow()][currentCell.getColumn()-1];
					return true;
				}
		...
		for(Cell[] c : cellsArr) {
			for(Cell cell: c) {
				if(cell.getRow()+1<rows) {
					cell.setSouth(cellsArr[cell.getRow()+1][cell.getColumn()]);
				}
				if(cell.getRow()-1>=0) {
					cell.setNorth(cellsArr[cell.getRow()-1][cell.getColumn()]);
				}
